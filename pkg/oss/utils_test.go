...
func TestQueryOssCoverURL(t *testing.T) {
	type args struct {
		objectKey string
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := QueryOssCoverURL(tt.args.objectKey)
			if (err != nil) != tt.wantErr {
				t.Errorf("QueryOssCoverURL() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("QueryOssCoverURL() got = %v, want %v", got, tt.want)
			}
		})
	}
}
